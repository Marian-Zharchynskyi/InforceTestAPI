...
public class UserByThisEmailAlreadyExistsException(UserId id)
    : AuthenticationException(id, $"User by this email already exists! User id: {id}");

public class EmailOrPasswordAreIncorrectException()
    : AuthenticationException(UserId.Empty(), "Email or Password are incorrect!");
